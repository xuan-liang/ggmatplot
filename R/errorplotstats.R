...
errorplotstats <- function(data, desc_stat = "") {
  if (desc_stat == "mean_se") {
    ...
    errorplot_data <- do.call(rbind, by(data, list(data$data$Group), function(x) {
      c(
        y = mean(x$x, na.rm = TRUE),
    ...
    errorplot_data <- do.call(rbind, by(data, list(data$Group), function(x) {
      c(
        ...
        ymin = quantile(x$x, probs = 0.25, na.rm = TRUE, names = FALSE),
        ymax = quantile(x$x, probs = 0.75, na.rm = TRUE, names = FALSE)
      )
    }))
