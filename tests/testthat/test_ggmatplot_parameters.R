...
test_that("line types updated based on linetype values", {
  ...
  expect_doppelganger("single linetype line plot", ggmatplot(y, plot_type = "line", linetype = "dashed"))
  # linetype values = number of groups
  expect_doppelganger("three linetype line plot", ggmatplot(x, y, plot_type = "line", linetype = c(15, 12, "dashed")))
  # linetype values < number of groups
  expect_error(ggmatplot(x, y, plot_type = "both", linetype = c(15, 12)), "Insufficient linetype values. 3 needed but only 2 provided.")
  # linetype values > number of groups
  expect_error(ggmatplot(x, y, plot_type = "both", linetype = c(15, 12, 3, 5)), "Too many linetype values. Only 3 needed but 4 provided.")
})
...
test_that("plot axes are transformed to a log scale based on log parameter values", {
  # log(x)
  expect_doppelganger("line plot with log x", ggmatplot(x,y, plot_type = "line", log = "x"))
  # log(y)
  expect_doppelganger("scatterplot with log y", ggmatplot(x,y, plot_type = "point", log = "y"))
  # log(x) and log(y)
  expect_doppelganger("point + line plot with log x and y", ggmatplot(x,y, plot_type = "both", log = "xy"))
})

test_that("plot title is updated based on main paramter", {
  expect_equal(ggmatplot(x,y, plot_type = "line")$labels$title, NULL)
  expect_equal(ggmatplot(x,y, plot_type = "line", main="title test")$labels$title, "title test")
})

test_that("plot axis labels are updated based on xlab and ylab paramters", {
  expect_equal(ggmatplot(x,y, plot_type = "line", xlab="xlab test")$labels$x, "xlab test")
