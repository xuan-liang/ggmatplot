test_that("converts a single matrix into long format", {
  ...
  expect_equal(outputList$data$Observation_number, unlist(lapply(1:nrow(x), function(var) rep(var, ncol(x)))))

})

test_that("converts two matrices(x and y) into long format (ncolx>ncoly & ncoly=1)", {
  ...
  expect_equal(outputList$data$x, c(1, 11, 31, 2, 12, 32, 3, 13, 33))
  expect_equal(outputList$data$Observation_number, unlist(lapply(1:nrow(x), function(var) rep(var, ncol(x)))))
  expect_equal(outputList$data$d, unlist(lapply(y$d, function(var) rep(var, ncol(x)))))
})

test_that("converts two matrices(x and y) into long format (ncolx<ncoly & ncolx=1)", {
  ...
  expect_equal(outputList$data$Observation_number, unlist(lapply(1:nrow(y), function(var) rep(var, ncol(y)))))
  expect_equal(outputList$data$d, unlist(lapply(x$d, function(var) rep(var, ncol(y)))))
})
...
test_that("matrices of different column sizes (!ncolx=1 & !ncoly=1) throws an error", {
