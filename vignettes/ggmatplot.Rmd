...
Each `plot_type` allowed by the `ggmatplot()` function is also built upon a `ggplot2 geom` (geometric object), and will therefore support additional parameters passed on to [`ggplot2 layers`](https://ggplot2.tidyverse.org/reference/layer.html). These are often aesthetics, used to set an aesthetic to a fixed value, like `size = 2` or `alpha = 0.5`.
...
[This list of examples](../index.html) includes other types of plots we can create using `ggmatplot`.
...
But you can directly use the wide format data with `ggmatplot`, and achieve the same result.
...
First, we can break the data down into two matrices as follows:
